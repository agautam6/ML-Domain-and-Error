...
GPR_only_in_domain_RF_model_errors = np.asarray([])
GPR_only_in_domain_RF_residuals = np.asarray([])
GPR_only_out_domain_RF_model_errors = np.asarray([])
GPR_only_out_domain_RF_residuals = np.asarray([])
...
for train_index, test_index in rkf.split(X_train_total):
	...
	RF.train_synth(X_train, y_train, std=standard_deviation)
	rf_pred, RF_errors = RF.predict_no_divide(X_test, True)
	rf_errors = (rfslope * (RF_errors/standard_deviation)) + rfintercept
	...
	for i in range(0, len(rf_errors)):
		if gpr_errors[i] < gpr_cutoff:
			GPR_only_in_domain_RF_residuals = np.append(GPR_only_in_domain_RF_residuals, rf_res[i])
			GPR_only_in_domain_RF_model_errors = np.append(GPR_only_in_domain_RF_model_errors, rf_errors[i])
		else:
			GPR_only_out_domain_RF_residuals = np.append(GPR_only_out_domain_RF_residuals, rf_res[i])
			GPR_only_out_domain_RF_model_errors = np.append(GPR_only_out_domain_RF_model_errors, rf_errors[i])
	# Increment counter
	outerctr = outerctr + 1


# Save all arrays
np.save('Full_Method_Diffusion_noise/step_2_arrays/GPR_only_in_domain_RF_model_errors', GPR_only_in_domain_RF_model_errors)
np.save('Full_Method_Diffusion_noise/step_2_arrays/GPR_only_in_domain_RF_residuals', GPR_only_in_domain_RF_residuals)
np.save('Full_Method_Diffusion_noise/step_2_arrays/GPR_only_out_domain_RF_model_errors', GPR_only_out_domain_RF_model_errors)
np.save('Full_Method_Diffusion_noise/step_2_arrays/GPR_only_out_domain_RF_residuals', GPR_only_out_domain_RF_residuals)